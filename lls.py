...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Show filepatterns of numbered files")
    parser.add_argument("-s",
                        "--size",
                        help="display size of file patterns",
                        action="store_true")
    parser.add_argument("-m",
                        "--minimal",
                        help="only show file patterns",
                        action="store_true")
    parser.add_argument(
        "-v",
        "--verbose",
...
def condensed_string(maskString, n):
    ...
    Args:
        maskString (string): The string to be condensed
        n (int): Condense to this unit size
    ...
    for grouping_value, group_items in itertools.groupby(maskString):
        ...
        condensedItemType = Color.UNDERLINE + Color.BOLD + Color.RED + str(
            itemType) + Color.END
...
def natural_keys(text):
    """Use for human sorted sort key. 

    Args:
        text ([type]): [description]

    Returns:
        [integers]: Human sorted integers
    """
...
def main():
    ...
    for k, v in filePatterns.items():
        ...
        if args.minimal:
            ...
            message = filePattern + "\t----> has range: " + str(
                minFrameNum) + "..."
            message += str(maxFrameNum) + "\tcompleted( " + str(
                len(filePatternMatches)) + " )"

        #adds ASCII representation of frames
        if args.verbose:
            verboseMessage = "\nframes: " + "".join(
                frames_ascii(minFrameNum, maxFrameNum, frames))
            if args.verbose > 1:
